...
#define NR_SPOT_LIGHTS 6  // Number of lampposts with spotlights
#define NR_SPOT_LIGHTS_Emit 2  // Number of lampposts with spotlights
...
uniform bool spotLightsOn[NR_SPOT_LIGHTS]; // Separate toggle for each spotlight

uniform SpotLightthatEmit spotlightthatemit[NR_SPOT_LIGHTS_Emit];
uniform bool spotlightthatemitOn = false;
...
vec3 CalcSpotLights(Material material, SpotLight light, vec3 N, vec3 Pos, vec3 V, bool isOn);
vec3 CalcSpotLightsthatemits(Material material, SpotLightthatEmit light, vec3 N, vec3 Pos, vec3 V, bool isOne);
...
void main()
{
    ...
    for (int i = 0; i < NR_SPOT_LIGHTS; i++) {
        if (spotLightsOn[i]) {
            result += CalcSpotLights(material, spotLights[i], N, Pos, V, spotLightsOn[i]);
        }
    }
    for (int i = 0; i < NR_SPOT_LIGHTS_Emit; i++) {
        if (spotlightthatemitOn) {
            result += CalcSpotLightsthatemits(material, spotlightthatemit[i], N, Pos, V, true);
        }
    }
