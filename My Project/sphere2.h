...
public:
    ...
    // Constructor
    Sphere2(float radius = 1.0f, int sectorCount = 36, int stackCount = 18,
    ...
    void set(float radius, int sectors, int stacks, glm::vec3 amb, glm::vec3 diff, glm::vec3 spec, float shiny)
    {
        ...
        if (sectors < MIN_SECTOR_COUNT2)
            this->sectorCount = MIN_SECTOR_COUNT2;
        this->stackCount = stacks;
        if (stacks < MIN_STACK_COUNT2)
            this->stackCount = MIN_STACK_COUNT2;
        this->ambient = amb;
        this->diffuse = diff;
        this->specular = spec;
