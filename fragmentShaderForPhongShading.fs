...
uniform int amb=1,diff=1,spec=1;
...
void main()
{
    ...
    vec3 emission = material.emission; // Directly add emissive intensity
    result += emission;
...
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V)
{
    ...
    //vec3 L = normalize(light.position - fragPos); // Light direction from fragment to light
//vec3 I = -L; // Incident vector towards the surface
//vec3 R = I - 2.0 * dot(I, N) * N; // Reflection vector
